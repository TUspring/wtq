        ...
        <div class="list_box">
            <table  border="1" width="100%">
                ...
                <tr v-for="value in listModal" v-if="listModal.length && !loading" >
                    ...
                    <td>{{value.transfer_voucher==0?'-':value.transfer_voucher}}</td>
                    <td>{{value.parities}}</td>
                    <td>{{value.RMB}}</td>
                </tr>
            </table>
            <!--=======加载中=====-->
    ...
    export default {
        data(){
            return{
                ...
                alreadyPresentedList:''        //返回数据
            }
        },
        computed:{
            listModal(){
                return this.alreadyPresentedList
            }
        },
        beforeMount(){
            this.getAlreadyPresentedList()
        },
        methods:{
            //时间戳转时间格式
            timeType (dateString) {
            ...
            //获取已提现列表
            getAlreadyPresentedList(page=1){
                this.loading = true
                var ajaxData = {
                    page,
                    type:1
                    ...
                    .then((response)=>{
                        ...
                        if(retCode == '0'){
                            ...
                            if(current_page!='1'&&!list.length){
                                this.alreadyPresentedList(--current_page)
                            }
                            Object.assign(this.pageIndex,pageIndexMsg)
                            this.alreadyPresentedList = list
                            this.loading = false
                        }
                    })
            },
            //分页
            changePage(page){
                this.getAlreadyPresentedList(page)
            },
        },
        activated(){
