...
<template>
    <div class="batchRefuseModal">
        <div class="top_content">批量拒绝提现申请</div>
        ...
        methods:{
            ...
            submit(){
                ...
                if(this.content.length >20){
                    this.$Modal.warning({title: `批量拒绝失败`, content: '拒绝理由理由不能超过20个字！'})
                    return
                }
                ...
                if(this.modalData.idList == 0){
                    this.$Modal.warning({title:`${httpName}失败`,content:'请先选择要拒绝用户！'})
                    return
                }
                var ajaxData = {
                ...
                this.$http.put('/admin/extract/audit_batch',ajaxData)
                    .then((response)=>{
                        ...
                        const {retCode} = data
                        if(retCode == '0'){
                            this.$Message.success({content: `批量拒绝成功`})
                            this.cancel()
                            this.Refresh()
                        }
                    }).catch((error) =>{
