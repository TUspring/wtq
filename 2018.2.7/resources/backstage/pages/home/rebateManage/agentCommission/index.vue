    ...
    <div class="layout-content-main">
        <div class="layout-breadcrumb">
            <Breadcrumb>
                ...
                <BreadcrumbItem>经纪人佣金表</BreadcrumbItem>
            </Breadcrumb>
        </div>
        ...
        <div class="examine-top">
            ...
            <div class="rightInfo">
                ...
                <div class="rightInfo-r">
                    <input type="text" placeholder="请输入经纪人姓名" v-model.trim="real_name" @keyup.enter="submit">
        ...
        <div class="list_box">
            <table  border="1" width="100%">
                ...
                <tr v-for="value in listModal" v-if="listModal.length && !loading">
                    ...
                    <td>{{value.income}}</td>
                    <td>{{value.direct_income}}</td>
                    <td>{{value.indirect_income}}</td>
                    <td>{{value.equal_income}}</td>
                </tr>
            </table>
            <!--=======加载中=====-->
    ...
    export default {
        data(){
            return{
                ...
                agentCommissionList:''    //返回数据
            }
        },
        computed:{
            listModal(){
                return this.agentCommissionList
            },
            ...mapState({
        ...
        methods:{
            ...
            search(){
                ...
                this.getAgentCommissionList(1)
            },

            //获取经纪人佣金表数据
            getAgentCommissionList(page=1){
                this.loading=true
                var ajaxData = {
                    page
                }
                ajaxData = Object.assign(ajaxData,this.searchKey)
                this.$http.get('/admin/brokerage/broker_brokerages',{params:ajaxData})
                    .then((response)=>{
                        ...
                        if (retCode == '0'){
                            ...
                            if (current_page != '1' && !list.length) {
                                this.getAgentCommissionList(--current_page)
                            }
                            Object.assign(this.pageIndex, pageIndexMsg)
                            this.agentCommissionList = list
                            this.loading=false
                        }
                    })
            },
            //分页
            changePage(page){
                this.getAgentCommissionList(page)
            },
