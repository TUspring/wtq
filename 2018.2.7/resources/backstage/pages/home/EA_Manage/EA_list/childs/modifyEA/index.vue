    ...
    <div class="layout-content-main">
        ...
        <div class="layout-breadcrumb">
            <Breadcrumb>
                ...
                <BreadcrumbItem>修改EA</BreadcrumbItem>
            </Breadcrumb>
        </div>
        ...
        <div class="EA-title">
            ...
            <Input style="width: 200px" v-model="EA_price"></Input>
            <span> 元/套</span>
        </div>
        <div class="EA-pic cl">
        ...
        <div class="EA-btn">
            ...
            <Button @click="modifyEA" type="primary">保存EA</Button>
        </div>
    </div>
</template>
<script>
    ...
    import jQuery_form from '../jQuery_form'
    export default {
        components: {},
        name: 'text-editor',
        data(){
        ...
        beforeMount() {
            ...
            this.EA_id=this.$route.params.id;
            this.getSingleEA()
        },
        mounted () {
            this.init();
        ...
        methods: {
            getSingleEA(){
                ...
                this.$http.get('/admin/ea_manage/' + this.EA_id)
                    .then((response) => {
                    const data = response.data;
                    const {retCode, retData = {}}=data;
                    if (retCode == '0') {
                        this.singleEAData=retData
                        this.EA_name=this.singleEAData.name
                        this.EA_price=this.singleEAData.price
                        this.EA_introduction=this.singleEAData.introduction
                       this.personalForm.avatar=this.singleEAData.images.path
                        tinyMCE.activeEditor.setContent(this.singleEAData.detail)
                    } else {
                    }
                }).catch((error) => {
                    console.log(error);
            ...
            modifyEA(){
                ...
                var titleName='修改EA失败'
                if(this.EA_name==''){
                    return   this.$Modal.warning({title: `${titleName}`, content: `EA名称不能为空！`})
                }
                if(this.personalForm.avatar==''){
                    return   this.$Modal.warning({title: `${titleName}`, content: `请上传EA封面图片！`})
                }
                if(this.EA_price==''){
                    return   this.$Modal.warning({title: `${titleName}`, content: `EA价格不能为空！`})
                }
                if(this.EA_introduction==''){
                    return   this.$Modal.warning({title: `${titleName}`, content: `EA简介不能为空！`})
                }
                if(EA_details==''){
                    return   this.$Modal.warning({title: `${titleName}`, content: `EA详情不能为空！`})
                }
                if(isNaN(this.EA_price)){
                    return   this.$Modal.warning({title: `${titleName}`, content: `EA价格只能为数字！`})
                }
                if(this.EA_price<0||this.EA_price>100000){
                    return   this.$Modal.warning({title: `${titleName}`, content: `EA价格为0.00-99999.99，保留小数点后两位！`})
                }
                ...
                return new Promise((resolve, reject) => {
                    this.$http.put('/admin/ea_manage/update_ea',ajaxData)
                        .then((response) => {
                        const data = response.data;
                        const {retCode, retData = {}}=data;
                        if (retCode == '0') {
                            this.spinLoading=false
                            this.$Message.success({content: `修改EA成功`})
                            this.$router.push({
                            ...
                            this.spinLoading=false
                            this.$Message.error({content: `修改EA失败`})
                            reject();
                        }
                    }).catch((error) => {
                        this.spinLoading=false
                        this.$Modal.error({title: `修改EA失败`, content: this.util.getHttpText(error)})
                        reject();
                    });
                })
            },
                    ...
                    tinymce.init({
                        ...
                        setup: function (editor) {
                            editor.on('init', function (e) {
                                vm.spinShow = false;
                                if (localStorage.modifyContent) {
                                 tinymce.get('tinymceEditer').setContent(localStorage.modifyContent);
                                }
                            });
                            editor.on('keyup', function (e) {
                                localStorage.modifyContent = tinymce.get('tinymceEditer').getContent();
