...
<template>
    <div class="historyRecord_box">
        ...
        <!--表格-->
        <div class="list_box">
            <table border="1" width="100%" v-if="showPage">
                <tr>
                    <th>订单号</th>
                    <th>策略名称</th>
                    <th>开仓时间</th>
                ...
                <tr  v-for="(value,index) in listModel" v-if="listModel.length">
                    <td>{{value.Ticket}}</td>
                    <td>
                        {{value.policy==null?'-':value.policy.name}}
                    </td>
                    <td>{{timeType(value.OpenTime)}} </td>
                    <td>
                        <span v-if="value.Type==1">卖出</span>
                        <span v-if="value.Type==0">买入</span>
                    </td>
                    <td>{{value.Lots}}</td>
                    <td>{{value.Symbol}} </td>
                    <td>{{value.OpenPrice}}</td>
                    <td>{{timeType(value.CloseTime)}}</td>
...
<script>
    export default {
        ...
        methods: {
            ...
            //获取统计平仓记录数据列表
            getCountCloseList(){
                this.$http.get('admin/count_close/'+this.dataId)
                    .then((response)=>{
                        const data = response.data
                        const {retCode,retData={}} = data
                        if(retCode == '0'){
            ...
            getMT4ListHttp(page=1){
                ...
                return new Promise((resolve, reject) => {
                    this.showPage=false;
                    this.$http.get('admin/close_detail/'+this.$parent.dataId,{params:ajaxData})
                        .then((res) => {
                            const data = res.data
                            const {retCode,retData={}}=data
                            if(retCode == '0'){
